...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # cuda()函数都能实现从CPU到GPU的内存迁移
        self.lr = opt.lr
        self.old_lr = opt.lr

        if self.isTrain:            
            self.netG.train()
            # 实例化一个loss_joint对象，并且这个对象下边可以当作一个函数被调用，这是类中__call__方法的意义所在。
    ...
    def forward(self):
        ...
        # netG.forward()是networks中的unetwrapper里的forward()函数
    ...
    # # 被上边 evlaute_iiw 调用，最终在测试 iiw 数据集的时候使用
    ...
    # # 被上边 evaluate_WHDR 调用，最终在测试 iiw 数据集的时候使用
    ...
    # # 在测试 saw 数据集 train 的 validation_SAW l97 调用
    ...
    # # 在测试 saw 数据集 train 的 validation_SAW l73 调用 
    ...
    # # 被上边 compute_pr 调用，最终 train 的 validation_SAW 调用
    ...
    # # 被上边 get_precision_recall_list_new 调用，最终 train 的 validation_SAW l73 调用
